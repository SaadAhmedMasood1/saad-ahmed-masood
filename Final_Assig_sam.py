# -*- coding: utf-8 -*-
"""
Created on Tue Feb 14 17:48:38 2023
...
st.title('DASHBOARD OF OLYMPIC GAME HISTORY')
st.header('Project Owner: Mr.Saad Ahmed Masood')

#Read data from for laptop
athlete_df=pd.read_csv(r'C:\Users\HP\Desktop\Karachi AI 2\Class\Class 1 (python)\4th day\Assignment Streamlit\athlete_events.csv')
noc_df=pd.read_csv(r'C:\Users\HP\Desktop\Karachi AI 2\Class\Class 1 (python)\4th day\Assignment Streamlit\noc_regions.csv')
...
with st.container():
    ...
    ax= plt.barh(medal_name.index, medal_name.values, color=['teal','cyan','gold','coral','tan','violet','brown','orange','maroon','red'])
    plt.xlabel('Numbers of Medals')
    Y.header('Top Ten Medalists')
    Y.pyplot(fig)
...
with st.container():
    ...
    T.header('Winner by Genders')
    T.pyplot(fig1)
    figb = plt.figure(figsize=(5,5))
    sns.barplot(season_medal.index,season_medal.values, alpha=1.0) # graph can be light/dark by change value of alpha min=0, max=1
    U.header('Season Victories')
    U.pyplot(figb)
